...
We move away from "template" looks by embracing **intentional asymmetry** and **overlapping geometry**. Elements should feel like they are floating in a pressurized, high-energy environment—anchored by deep navy tones but electrified by emerald pulses. We use subtle rounded corners (roundedness: 1) to communicate technical accuracy and "Glassmorphism" to provide a sense of atmospheric depth.
...
*   **Neutral (`#FCF8FA`):** Our light-mode base, providing a "clean-room" clinical feel.
...
### Buttons
*   **Primary:** Background: `secondary` (Cyber Emerald); Text: `on-secondary-fixed` (#002203). Subtle rounded corners (roundedness: 1). High-energy, high-visibility.
...
### Cards & Lists
*   **Forbid Dividers:** Do not use horizontal lines to separate list items. Use **Vertical White Space** (Spacing Scale 2: 1rem) or alternating background shifts between `surface` and `surface-container-low`.
...
*   **Do** lean into the subtle corner radius (roundedness: 1); it creates a "machined" look that feels more premium than fully rounded or fully square edges.
